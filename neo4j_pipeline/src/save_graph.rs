use base_types::{common::*, primitives::PrimitiveValues};
use neo4j_pipeline_generated_toolkit::prelude::*;
use neo4rs::Graph;
pub async fn save_graph(payload: Vec<StandaloneRGSOConcrete>) {
    ...
    let (node_creation_queries, relationship_creation_queries)= payload.iter().fold((Vec::new(), Vec::new()),|mut agg, node| {
     ...
     // Base query
        ...
        for (key, value) in &node.fields {
            let string_id = base_types::common::u128_to_string(*key);
            // creation_query = creation_query.param(&string_id, string_id.clone());
...
,
    ...
    // txn.run_queries(node_creation_queries).await.unwrap();
    // txn.run_queries(relationship_creation_queries).await.unwrap();
