...
//     };
#[derive(Debug)]
pub struct Blueprint<TSchema: RGSO<Schema = TSchema>> {
    instantiables: Vec<Rc<dyn RInstantiable<Schema = TSchema>>>,
    add_child_updates: Vec<(Uid, SlotRef)>,
    add_parent_updates: Vec<(Uid, SlotRef)>,
    remove_child_updates: Vec<(Uid, SlotRef)>,
    remove_parent_updates: Vec<(Uid, SlotRef)>,
    deleted_instances: Vec<Uid>,
    field_updates: Vec<(Uid, HistoryFieldEdit)>,
    action_tag: Option<TaggedAction>,
}
struct UpdateSnapshot<'a> {
    added_children: Vec<&'a SlotRef>,
    // added_parents: &SlotRef,
    removed_children: Vec<&'a SlotRef>,
    // removed_parents: &SlotRef,
}
impl<TSchema: RGSO<Schema = TSchema>> Blueprint<TSchema> {
    fn check_blueprint(&self, graph: &RBaseGraphEnvironment<TSchema>) -> Result<(), Error> {
        Ok(())
    }
}
...
// RGSOWrapperBuilder<T, TSchema>: RProducable<RGSOWrapper<T, TSchema>>,
{
    ...
    deleted_instances: RwSignal<Vec<Uid>>,
    ...
    T: RIntoSchema<Schema = TSchema> + Clone + std::fmt::Debug + 'static,
{
    ...
    pub fn execute(&self) -> Result<(), Error> {
        ...
        blueprint.check_blueprint(&self.graph)?;
    ...
    fn get_blueprint(self) -> Blueprint<TSchema> {
        let mut new_instantiables = self.instantiables.get();
        if let Some(instance) = self.wip_instance {
            new_instantiables.push(Rc::new(instance));
        }
        Blueprint::<TSchema> {
            instantiables: new_instantiables,
            ...
            deleted_instances: self.deleted_instances.get(),
    ...
    fn new(
        ...
        Self {
            ...
            deleted_instances: RwSignal::new(Vec::new()),
        }
    }
    ...
    fn remove_child_from_slot(&mut self, slot_ref: SlotRef) {
        self.remove_child_updates
            .update(|prev| prev.push((slot_ref.host_instance_id, slot_ref.clone())));
        self.remove_parent_updates
            .update(|prev| prev.push((slot_ref.child_instance_id, slot_ref)));
    }
